...
class ArgoDataset(Dataset):
    def __init__(self, split, config, train=True):
        ...
        if 'preprocess' in config and config['preprocess']:
            if train:
                ...
                self.split = self.config['preprocess_val']
        else:
    ...
    def __getitem__(self, idx):
        if 'preprocess' in self.config and self.config['preprocess']:
            ...
            if self.train and self.config['rot_aug']:
                ...
                new_data = dict()
                for key in ['city', 'orig', 'gt_preds', 'has_preds', 'theta', 'rot', 'feats', 'feat_locs', 'ctrs', 'graph']:
                    if key in data:
                        new_data[key] = ref_copy(data[key])
                data = new_data
    ...
    def get_obj_feats(self, data):
        ...
        feats, feat_locs, ctrs, gt_preds, has_preds = [], [], [], [], []
        for traj, step in zip(data['trajs'], data['steps']):
            if 19 not in step:
                continue


            feat_loc = np.zeros((50, 2), np.float32)
            feat_loc[step] = np.matmul(rot, (traj - orig.reshape(-1, 2)).T).T
        ...
        if 'preprocess' in self.config and self.config['preprocess']:
            ...
            if self.train and self.config['rot_aug']:
                ...
                rot = np.asarray([
                    ...
                    [np.sin(-dt), np.cos(-dt)]], np.float32)
                new_data['feats'] = data['feats'].copy()
                new_data['feat_locs'] = data['feat_locs'].copy()#修改
                new_data['feats'][:, :, :2] = np.matmul(new_data['feats'][:, :, :2], rot)
                new_data['ctrs'] = np.matmul(data['ctrs'], rot)
                ...
                new_data = dict()
                for key in ['orig', 'gt_preds', 'has_preds', 'theta', 'rot', 'feats', 'feat_locs', 'ctrs', 'graph','argo_id','city']:
                    if key in data:
                        new_data[key] = ref_copy(data[key])
                data = new_data
            return data
