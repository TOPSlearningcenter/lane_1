...
def preprocess_dataset(dataset_name, dataset_path, hist_len, fut_len, output_path):
    # Import all settings for experiment.
    args = parser.parse_args()
    model = import_module(args.model)
    # print(args.model)
    config, *_ = model.get_model()
    # print("preprocess_dataset",config)
...
def train(config):
    ...
    stores = [None for x in range(779)]#修改：205942
    t = time.time()
...
def val(config):
    ...
    stores = [None for x in range(191)] #修改：39472
...
def test(config):
    ...
    stores = [None for x in range(191)]#修改：78143
